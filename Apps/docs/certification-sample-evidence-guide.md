...
此指南已建立，可為 isv 提供每個 Microsoft 365 認證控制項所需的證據類型和詳細程度的範例。 在本檔中共用的任何範例都不代表可用於示範控制措施符合的唯一證據，但是只會做為所需證據類型的指導方針。 
...
這份檔會直接對應至您在夥伴中心的憑證期間會出現的控制項。 本檔中提供的指導方針如下所述：
...
**注意：** 這個螢幕擷取畫面顯示原則/程式檔，預期是供 Isv 共用實際的支援原則/程式檔，而不只是提供螢幕擷取畫面。
...
**注意：** 提供的證據必須包含匯出記錄檔，以在較長的時間內顯示每日更新。 某些防病毒產品會產生更新記錄檔，所以應該提供這些檔案，或從事件檢視器匯出記錄。

**控制項4：** 提供 demonstratable 證據，以將反病毒設定為執行跨存取掃描或所有抽樣系統元件的定期掃描。

**注意：** 如果未啟用 [存取時掃描]，則 **必須** _be 啟用「每日掃描」和「alerting_。
...
**注意：** 這個螢幕擷取畫面顯示檔，預期是供 Isv 共用實際的支援檔，而不只是提供螢幕擷取畫面。
...
**注意** 即使您是在純平臺內的服務環境中執行，仍然有責任識別您的程式碼基底中的漏洞：例如，協力廠商文件庫。
...
**注意：** 這個螢幕擷取畫面顯示原則/程式檔，預期是供 Isv 共用實際的支援原則/程式檔，而不只是提供 screenshot。_
...
**注意** 如果使用 Nessus，則必須定期執行此做法，以快速識別安全性漏洞。 我們建議您至少每週更新一次。
...
**注意：** 所有範圍內系統元件的修補都必須是證據。 這包括類似事項;作業系統更新、Application/Component 更新 (i.e__。、 _Apache Tomcat、OpenSSL 等 ) 、軟體相依性 (例如 JQuery、AngularJS_ 等 ) 等等。
...
**注意：** 針對此控制項，認證分析人員必須在過去的12個月內，查看每個季度的弱點掃描報告和修正。
...
**注意：** 證據應該示範環境所使用的所有公開 Ip，以確保所有的進入點都涵蓋，也就是這個螢幕擷取畫面也包含的原因。
...
**注意：** 您可以在開發/測試環境中使用即時資料，提供開發/測試以包含在評估範圍內，這樣就能對照 Microsoft 365 憑證控制來評估安全性。
...
**注意：** 下列螢幕擷取畫面顯示安全的軟體發展檔，預期是供 Isv 共用實際的支援檔，而不只是提供螢幕擷取畫面。
...
**注意：** 您將需要提供所有非主控台連線的證據，以示範是否已為 MFA 啟用 MFA。 例如，如果您的 RDP 或 SSH 是伺服器或其他系統元件 (亦即，防火牆) 。
...
**注意：** IP 位址是私人 RFC 1918 位址，未公開路由。 這只是為了示範的目的。
...
**注意：** 如果 SIEM 是內部的，將需要提供證據，以示範對記錄資料的存取是否限制為根據其工作需求選取的使用者人數，而且該平臺本身會受到保護，以避免受到篡改 (大多數的解決方案都會將此功能構建成記錄解決方案) 的功能。
...
**注意：** 應該提供 assessment_ _document__ation 的完整風險，而不是螢幕擷取畫面。
...
**注意：** 需要共用完整的報表。 此練習也可以在內部執行，因為這不是由協力廠商公司執行的 Microsoft 365 需求。
...
**注意：** 若未使用協力廠商，則必須以資深領導小組成員的身分 (電子郵件) 來確認。
