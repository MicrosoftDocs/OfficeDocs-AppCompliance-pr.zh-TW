...
寫入存取需要從規則傳送郵件 (例如自動回復) ，以及使用 Emailgistics 交談識別碼更新郵件主體。 |
>| 傳送郵件 | 委託 | 允許 Emailgistics 從共用信箱傳送電子郵件。 若要從規則 (（例如自動回復) ）傳送郵件，則需要此許可權。 |
>| MailboxSettings ReadWrite | 委託 | 可讓 Emailgistics 取得共用信箱名稱，並建立信箱規則，將內送郵件路由至其隱藏的內送資料夾，以進行初始處理、規則執行和自動指派。 |
>| User.Read | 委託 | 允許 Emailgistics 登入共用信箱，並閱讀相關的基本資訊，例如服務主體識別碼、電子郵件地址和使用者名稱。 |
>| User.ReadBasic.All | 委託 | 允許 Emailgistics 讀取您租使用者中使用者的基本資訊。 需要在 Emailgistics 中偵測信箱成員的電子郵件地址和名稱中的變更，並自動更新此資訊，以用於追蹤和報告目的。 這是其範圍延伸至共用信箱以外的唯一許可權。 |
>| offline_access | 委託 | 允許 Emailgistics 做為後端應用程式，而不是互動式瀏覽器型應用程式，需要使用者在每次運作時登入。 此許可權可讓 Emailgistics 取得重新整理權杖，視需要重新整理，以維護持續存取。 |
