---
title: sapiens 的應用程式資訊，由 SharePointSapiens SharePoint | 現代使用者介面
ms.author: elmalova
author: elenamalova
manager: tonybal
ms.date: 03/31/2022
...
description: 所有適用于 SharePoint | sapiens 現代化使用者介面的安全性和符合性資訊資訊，其資料處理原則，其 Microsoft Cloud App Security 應用程式目錄資訊，以及 CSA 星型登錄中的安全性/符合性資訊。
...
---
# <a name="sharepointsapiens-modern-user-interface"></a>SharePoint | sapiens 現代使用者介面
...
由 SharePointSapiens 提供給 Microsoft 的資訊：

| **Information** | **回應** |
|:----------------|:-------------|
| 應用程式名稱 | SharePoint sapiens 現代化使用者介面 |
| ID | WA200003529 |
| 支援 Office 365 用戶端 | SharePoint 2016 或更新版本 |
| 合作夥伴公司名稱 | SharePointSapiens |
...
| 應用程式的核心功能 | 此應用程式必須與我們的 SharePoint 搭配使用|sapiens 事件管理 app (WA104380834) 或 SharePoint|sapiens 員工訓練管理應用程式 (WA104380833) 。  這個應用程式提供直觀的使用者介面，可讓您在 SharePoint 中建立及管理事件、主題、課程和 curriculums、註冊使用者或讓使用者自行註冊，以及追蹤所有出席者、工作分派和成就。 |
...
此資訊是由 SharePointSapiens 提供的，此應用程式會如何收集和儲存組織資料，以及您的組織將會擁有該應用程式所收集資料的控制項。
...
>|:------------------------|:--------------------|:------------------------------------|:--------------------|
>| User.ReadBasic.All | 委託 | 讀取基本的使用者資訊以啟用 Exchange 會議室預約 | [9e8e113c-8a08-4606-b08a-de4decc7252f](../azure/9e8e113c-8a08-4606-b08a-de4decc7252f.md) |
