...
| App 的使用條款 | [https://www.dbit.io/termsofuse.html](https://www.dbit.io/termsofuse.html) |
| 應用程式的核心功能 | 以較少的麻煩 DBit 密碼管理解決方案。 DBit 是小型至大型小組的密碼管理工具。 透過 DBit，您可以安全地共用小組成員之間的密碼。 產生的共用密碼和密碼會在設定的持續時間過後自動過期。 您的密碼是在我們的伺服器上的記憶體中加密的，永遠不會儲存至磁片存放區。 一旦密碼到期，便會將其從我們的伺服器記憶體中移除。 此應用程式是由 Diamondsoft 解決方案所開發，LLC |
