...
>|:----------------|:--------------------|:---------------------------------------------------|:--------------------------|:--------------------------|
>| User.Read | 委託 | 動態信號會將使用者從 Azure AD 同步處理至其平臺，以允許即時簡化及停用使用者。 資料會儲存在動態信號內，以允許使用者在同步處理的情況下使用該應用程式。 | 特定使用者與 Azure AD 同步處理動態信號平臺使用者的讀取權限。 | 79ff4a2a-e22b-47d5-94dc-ef76fe46af75 |
>| User.Read.All | 委託 | 動態信號會將使用者從 Azure AD 同步處理至其平臺，以允許即時簡化及停用使用者。 資料會儲存在動態信號內，以允許使用者在同步處理的情況下使用該應用程式。 | 特定使用者與 Azure AD 同步處理動態信號平臺使用者的讀取權限。 | 79ff4a2a-e22b-47d5-94dc-ef76fe46af75 |
>| offline_access | 委託 | 動態信號會將使用者從 Azure AD 同步處理至其平臺，以允許即時簡化及停用使用者。 資料會儲存在動態信號內，以允許使用者在同步處理的情況下使用該應用程式。 | 保留對租使用者群組和團隊的存取權。 | 79ff4a2a-e22b-47d5-94dc-ef76fe46af75 |
>| openid | 委託 | 動態信號會將使用者從 Azure AD 同步處理至其平臺，以允許即時簡化及停用使用者。 資料會儲存在動態信號內，以允許使用者在同步處理的情況下使用該應用程式。 | 使用動態信號應用程式驗證使用者。 | 79ff4a2a-e22b-47d5-94dc-ef76fe46af75 |
