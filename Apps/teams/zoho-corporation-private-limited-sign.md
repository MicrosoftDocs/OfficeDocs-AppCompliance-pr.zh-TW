...
| App 的使用條款 | [https://www.zoho.com/terms.html](https://www.zoho.com/terms.html) |
| 應用程式的核心功能 | Zoho 符號是一種安全、雲端型的數位簽章應用程式，可讓您以法律和電子方式簽署、傳送和管理您的商務文書工作。 您可以輕鬆地建立電子簽署工作流程來收集簽章、驗證簽署者、觸發提醒，以及即時追蹤簽章和核准狀態。 Zoho 符號也可讓您直接從網頁瀏覽器和使用行動應用程式，直接從 Microsoft 應用程式管理所有的書面。 |
...
| 如果基礎 infastructure 處理或儲存 Microsoft 客戶資料，該資料地理位置存放在何處？ | 印度，日本，澳大利亞，中國，荷蘭 (美國的) ，愛爾蘭，美國 |
...
| 您的應用程式是否支援條件式存取原則？ | 是 |
| 列出支援的原則類型 | Multi-Factor 驗證，需要特定應用程式的組織管理裝置，封鎖危險登入行為，限制提供給非系統管理角色之人員的存取權  |
